...
export const BookActions: React.FC<BookActionsProps> = ({
  ...
  const { data: customBook, mutate } = useSWR<{ book: CustomBook }>(
    `${rootPath}/api/book/${book.id}`
  );
  ...
  const isRead =
    currentBook.read && currentBook.read.find((id) => id === userId);
  const isfavourite =
    currentBook.favourite && currentBook.favourite?.find((id) => id === userId);
  ...
  const handleAddBook = async (type: BookTypeArg) => {
    ...
    const bookRes = await fetch(`${rootPath}/api/book/${currentBook.id}`, {
      ...
      body: JSON.stringify({
        ...currentBook,
        [type]: existingType.find((id) => id === userId)
          ? existingType.filter((id) => id !== userId)
          : [...existingType, userId],
      }),
    }).catch((err) => console.log("err when adding book", err));
  ...
  return (
    <Suspense fallback={<CircularProgress />}>
      <Tooltip title={!userId ? "zaloguj się aby móc tworzyć kolekcje" : ""}>
        <Box
          className={className}
          sx={{
            display: "flex",
